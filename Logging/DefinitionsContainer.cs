...
namespace MapleShark
{
    public sealed class DefinitionsContainer
    {
        ...
        private Dictionary<byte, Dictionary<uint, List<Definition>>> _definitions = new Dictionary<byte, Dictionary<uint, List<Definition>>>();
        ...
        {
            if (!_definitions.ContainsKey(pDefinition.Locale)) _definitions.Add(pDefinition.Locale, new Dictionary<uint, List<Definition>>());
        ...
        {
            Dictionary<ushort, Dictionary<uint, SortedDictionary<ushort, string>>>[] headerList = new Dictionary<ushort, Dictionary<uint, SortedDictionary<ushort, string>>>[2];
                ...
                foreach (var kvpVersion in kvpLocale.Value)
                {
                    foreach (var d in kvpVersion.Value)
                    {
                        ...
                        if (!headerList[outbound].ContainsKey(d.Locale))
                            headerList[outbound].Add(d.Locale, new Dictionary<uint, SortedDictionary<ushort, string>>());
